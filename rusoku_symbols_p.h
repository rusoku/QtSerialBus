...
#define OPTION_CAN_2_0_ONLY 0

/** @brief       CAN Bit-rate Settings (nominal and data):
 */
typedef union can_bitrate_t_ {
    quint32 index;                      /**< index to predefined bit-rate (<= 0) */
    struct {                            /*   bit-timing register: */
        quint32 frequency;              /**<   clock domain (frequency in [Hz]) */
        struct {                        /*     nominal bus speed: */
            quint16 brp;               /**<     bit-rate prescaler */
            quint16 tseg1;             /**<     TSEG1 segment */
            quint16 tseg2;             /**<     TSEG2 segment */
            quint16 sjw;               /**<     synchronization jump width */
            quint8  sam;               /**<     number of samples (SJA1000) */
...
GENERATE_SYMBOL_VARIABLE(int, can_test, uint8_t, const void *, int)
GENERATE_SYMBOL_VARIABLE(int, can_init, uint8_t, const void *)
GENERATE_SYMBOL_VARIABLE(int, can_exit, int)
GENERATE_SYMBOL_VARIABLE(int, can_kill, int)
GENERATE_SYMBOL_VARIABLE(int, can_start, const can_bitrate_t)
...
inline bool resolvePeakCanSymbols(QLibrary *rusokuLibrary)
{
    if (!rusokuLibrary->isLoaded()) {
        #ifdef Q_OS_MACOS
            rusokuLibrary->setFileName(QStringLiteral("libTouCAN"));
        #else
            pcanLibrary->setFileName(QStringLiteral("libTouCAN"));
        #endif
